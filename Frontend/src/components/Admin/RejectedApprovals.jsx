    ...
    try {
      const exportData = approvals.map((approval) => ({
        ...
        "Created At": approval.createdAt
          ...
          : "N/A",
        "Rejected Info At": approval.updatedAt
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Rejected Approvals");

      XLSX.writeFile(
        wb,
        `rejected_approvals_export_${
        ...
        sections: [
          {
            children: [
              new Paragraph({
                text: "Rejected Approvals Report",
                heading: "Heading1",
                spacing: { after: 400 },
              }),
              new Paragraph({
      ...
      >
        <div>
          <h1 className="text-2xl font-bold text-[#00539C]">
            {deptName ? `${deptName} - Rejected Info` : "Department Dashboard"}
          </h1>
          <p className="text-gray-600 mt-1 text-sm">
            Rejected Requests by Account Dept
          </p>
        </div>
